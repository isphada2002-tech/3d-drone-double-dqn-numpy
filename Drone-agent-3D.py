...
tif_filename = 'output_AW3D30.tif'
...
try:
    ...
    with rasterio.open(tif_filename) as src:
        ...
        Z_flip = np.flipud(elevation).astype(float)
    print(f"--> [3] Matrix extracted successfully with shape: {Z_flip.shape}")

    # Convert entire dimensions to percentages (0 to 100)
    y_dim, x_dim = Z_flip.shape
    x = np.linspace(0, 100, x_dim)
    y = np.linspace(0, 100, y_dim)
    X, Y = np.meshgrid(x, y, indexing='xy')

    # Normalize the Z-height to be geometrically consistent between (0 and 25)
    z_min, z_max = np.min(Z_flip), np.max(Z_flip)
    Z_flip = ((Z_flip - z_min) / (z_max - z_min)) * 25.0

    # Calculate how many of the 100 dimensions each pixel represents
    resolution_x = 100.0 / x_dim
    resolution_y = 100.0 / y_dim
...
surface['elevation'] = Z_flip.ravel()
surface.set_active_scalars("elevation")
...
base_model = surface.extrude((0, 0, -10), capping=True)
contours = surface.contour(isosurfaces=20, scalars=surface.points[:, 2])

plotter.add_mesh(surface, scalars="elevation", cmap="terrain", smooth_shading=True, ambient=0.2, diffuse=0.8, specular=0.1)
plotter.add_mesh(base_model, color="#4A4A4A", opacity=0.8, smooth_shading=True)
...
num_steps = 150
x_true = np.linspace(0, 100, num_steps)
y_true = np.linspace(0, 100, num_steps)

# Make the elevation higher than the highest peak by only 5 degrees
z_true = []
for i in range(num_steps):
    idx_x = int(min(max(x_true[i] / resolution_x, 0), Z_flip.shape[1] - 1))
    idx_y = int(min(max(y_true[i] / resolution_y, 0), Z_flip.shape[0] - 1))
    z_true.append(Z_flip[idx_y, idx_x] + 6.0)
...
for episode in range(20):
    ...
    if should_render:
        plotter.add_lines(true_points, color='orange', width=5, label='Planned Route')
        drift_line_actor = None
        drone_actor = plotter.add_mesh(pv.Sphere(radius=1.5, center=true_points[0]), color='grey')
        print(f"\n--> [10] SHOWING FINAL LIVE SIMULATION FOR EPISODE {episode + 1}/20...")
    elif (episode + 1) % 10 == 0:
        print(f"--> Training in background... Episode {episode + 1}/20 complete.")
    ...
    for step in range(num_steps):
        ...
        actual_x = max(0.0, min(100.0, tx + current_drift_x))
        actual_y = max(0.0, min(100.0, ty + current_drift_y))
        actual_z = tz + current_drift_z

        # Extract local ground elevation index underneath the drone

        idx_x = int(min(max(actual_x / resolution_x, 0), Z_flip.shape[1] - 1))
        idx_y = int(min(max(actual_y / resolution_y, 0), Z_flip.shape[0] - 1))
        ...
        if step >= gps_loss_step:
            ...
            current_drift_x += np.random.normal(0, 1.2)
            current_drift_y += np.random.normal(0, 1.2)
            current_drift_z += np.random.normal(0, 0.6)  # Vertical wind effect
            ...
            next_drift_x = current_drift_x + np.random.normal(0, 0.1)
            next_drift_y = current_drift_y + np.random.normal(0, 0.1)
            next_drift_z = current_drift_z + np.random.normal(0, 0.05)

            next_dist_err = np.sqrt(next_drift_x**2 + next_drift_y**2)

            # Future sensor simulation step for temporal difference target
            next_idx_x = int(min(max((tx + next_drift_x) / resolution_x, 0), Z_flip.shape[1] - 1))
            next_idx_y = int(min(max((ty + next_drift_y) / resolution_y, 0), Z_flip.shape[0] - 1))
            next_clearance = (tz + next_drift_z) - Z_flip[next_idx_y, next_idx_x]

            # 🛠 Compound 3D Reward Shaping Function
            reward = - (next_dist_err ** 2) * 2.0

            improvement = current_dist_err - next_dist_err
            reward += improvement * 15.0

            if next_dist_err < 1.5:
                reward += 30.0 * (1.5 - next_dist_err)
            

            # 🛑 CRITICAL CRASH AVOIDANCE CONSTRAINT LOGIC
            if next_clearance < 4.0:  # Severe mountain proximity limit
                reward -= 400.0         # Heavy mathematical penalty applied
            elif next_clearance < 6.5:
                reward += 50.0          # Safe cruise bonus reward
            else:
                reward += 5.0
        ...
        if should_render:
            ...
            drone_actor.mapper.dataset.copy_from(pv.Sphere(radius=1.5, center=[actual_x, actual_y, actual_z]))
            plotter.camera.azimuth += 0.25
            plotter.update()
            time.sleep(0.02)

    # Exploration rate decay step
    ai_agent.epsilon = max(0.01, ai_agent.epsilon * 0.97)
